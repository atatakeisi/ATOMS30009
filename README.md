...
`192.168.55.27` を開くと**2つのビュー**があり、最上部のボタンで切り替える。
ボタンはカテゴリで色分けし、**有効中は緑**で上書き表示する。
...
──── 姿勢調節 ────
   ...
   動作   [ 初期姿勢 真っ直ぐ ]   ← CALIB(水平保持)

   ┌────────┬────────┬────────┐
   │ 足踏み │ 前進   │ 屈伸   │
   ├────────┼────────┼────────┤
   │ 左旋回 │ 立ち   │ 右旋回 │   ← 中央=立ち(STAND)
   ├────────┼────────┼────────┤
   │ 反転   │ 後進   │ジャンプ│
   └────────┴────────┴────────┘
   [ 水平維持（IMU） ]
...
| 表示 | モード | 色 |
|------|--------|----|
| 立ち | STAND(IK立ち姿勢) | 青(中央) |
| 初期姿勢 真っ直ぐ | CALIB(全サーボ水平保持) | グレー |
| 足踏み/前進/屈伸/後進 | Step/Advance/Stretch/Back | 水色 |
| 左旋回/右旋回 | L/R | 黄 |
| 水平維持 | IMU(傾き補正) | 紫 |
| 反転/ジャンプ | Roll/Jump | 赤(危険) |

動作ボタンは押すと該当モードへ。同じボタン再押しで停止(立ちへ)。
...
```
   ...
   サーボ速度 [-][値][+]   [設定を初期値へ]
   初期姿勢 真っ直ぐ（水平保持）
   ID選択 [◀ID][ID 値][ID▶]   ← 脚・関節ラベル表示
   オフセット [-10][-1][値][+1][+10]
   生値 raw [値]   [このIDをリセット]
```

準備設定に入ると自動で **真っ直ぐ(CALIB=水平保持)** になり、オフセット変更が即サーボに反映されて見える。
「設定を初期値へ」は制御パラメータをデフォルトに戻す(period/upHeight/stride/speed/Kp/Kd/wakeUpAngle)。**offset(較正値)は消さない**。

---

## キャリブレーション手順

1. **準備設定へ →** を押す(自動で水平保持=CALIBになる)。
2. `ID選択` で ID を選び、`-10/-1/+1/+10` でそのサーボだけ動かす。
   どの脚のどの関節が、どちらに動くかを1個ずつ確認できる。
3. すべての脚が水平になるよう offset を調整(値は即 NVS 保存)。
4. **操作にもどる → 立ち(STAND)** で正しく立つか確認。
